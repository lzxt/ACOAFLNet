...
#define ACO_INITIAL_PHEROMONE 1.0
#define ACO_MIN_PHEROMONE 0.01
#define ACO_MAX_PHEROMONE 10000.0
#define ACO_EVAPORATION_RATE 0.05
#define ACO_ALPHA 1.0
#define ACO_BETA 2.0
#define ACO_CODE_REWARD 8.0
#define ACO_STATE_REWARD 20.0
#define ACO_UNIQUE_CRASH_REWARD 160.0
#define ACO_INVALID_PATH_PENALTY 0.85
#define ACO_TIMEOUT_PATH_PENALTY 0.70
#define ACO_STAGNATION_LIMIT 256
#define ACO_EPSILON_START 20
#define ACO_EPSILON_MIN 3

typedef struct {
  u32 from;
  u32 to;
  double pheromone;
  double depth_heuristic;
  u32 depth;
  u32 traversals;
  u32 successes;
  u32 failures;
  u32 code_discoveries;
  u32 state_discoveries;
  u32 unique_crashes;
} aco_transition_t;

static aco_transition_t *aco_transitions = NULL;
static u32 aco_transition_count = 0;
static u32 aco_transition_capacity = 0;
static double *aco_state_pheromones = NULL;
static u32 aco_state_pheromone_count = 0;
static u8 aco_last_state_new = 0;
static u8 aco_last_code_cov_new = 0;
static u8 aco_last_unique_crash = 0;
static u32 aco_current_state_id = 0;
static u32 aco_last_from_state_id = 0;
static u32 aco_last_to_state_id = 0;
static u64 aco_updates = 0;
static u32 aco_stagnation = 0;
static FILE *aco_log_file = NULL;

u32 get_state_index(u32 state_id);

static void aco_reset_last_feedback(void) {
  aco_last_state_new = 0;
  aco_last_code_cov_new = 0;
  aco_last_unique_crash = 0;
}

static void aco_open_log(void) {
  u8 *fn;

  if (aco_log_file || !out_dir) return;

  fn = alloc_printf("%s/aco_log.csv", out_dir);
  aco_log_file = fopen(fn, "a");
  if (aco_log_file) {
    fseek(aco_log_file, 0, SEEK_END);
    if (ftell(aco_log_file) == 0) {
      fprintf(aco_log_file, "update,execs,from,to,target,reward,code_cov,state_cov,unique_crash,valid,pheromone,depth_heuristic,epsilon,stagnation,paths_total,crashes_unique\n");
      fflush(aco_log_file);
    }
  }
  ck_free(fn);
...
static void aco_log_update(u32 from, u32 to, double reward, u8 new_code_cov, u8 new_state_cov, u8 unique_crash, u8 valid, aco_transition_t *transition) {
  u32 epsilon;

  if (state_selection_algo != ACO_SELECTION) return;

  aco_open_log();
  if (!aco_log_file) return;

  epsilon = ACO_EPSILON_MIN + (ACO_EPSILON_START - ACO_EPSILON_MIN) / (1 + (u32)(queue_cycle / 2));
  fprintf(aco_log_file, "%llu,%llu,%u,%u,%u,%.3f,%u,%u,%u,%u,%.6f,%.6f,%u,%u,%u,%llu\n",
          aco_updates, total_execs, from, to, target_state_id, reward, new_code_cov, new_state_cov,
          unique_crash, valid, transition ? transition->pheromone : 0.0,
          transition ? transition->depth_heuristic : 0.0, epsilon, aco_stagnation, queued_paths, unique_crashes);
  fflush(aco_log_file);
}

static void aco_ensure_state_pheromones(void) {
  if (aco_state_pheromone_count < state_ids_count) {
    u32 old_count = aco_state_pheromone_count;
    aco_state_pheromones = (double *)ck_realloc(aco_state_pheromones, state_ids_count * sizeof(double));
    while (old_count < state_ids_count) aco_state_pheromones[old_count++] = ACO_INITIAL_PHEROMONE;
    aco_state_pheromone_count = state_ids_count;
  }
}

static u32 aco_find_transition(u32 from, u32 to) {
  u32 i;
  for (i = 0; i < aco_transition_count; i++) {
    if (aco_transitions[i].from == from && aco_transitions[i].to == to) return i;
  }
  return aco_transition_count;
}

static aco_transition_t *aco_get_transition(u32 from, u32 to) {
  u32 idx = aco_find_transition(from, to);

  if (idx == aco_transition_count) {
    if (aco_transition_count == aco_transition_capacity) {
      aco_transition_capacity = aco_transition_capacity ? aco_transition_capacity * 2 : 64;
      aco_transitions = (aco_transition_t *)ck_realloc(aco_transitions, aco_transition_capacity * sizeof(aco_transition_t));
    }

    aco_transitions[idx].from = from;
    aco_transitions[idx].to = to;
    aco_transitions[idx].pheromone = ACO_INITIAL_PHEROMONE;
    aco_transitions[idx].depth = 0;
    aco_transitions[idx].depth_heuristic = 1.0;
    aco_transitions[idx].traversals = 0;
    aco_transitions[idx].successes = 0;
    aco_transitions[idx].failures = 0;
    aco_transitions[idx].code_discoveries = 0;
    aco_transitions[idx].state_discoveries = 0;
    aco_transitions[idx].unique_crashes = 0;
    aco_transition_count++;
  }
...
static void aco_refresh_transition_depth(aco_transition_t *transition) {
  ...
  k = kh_get(hms, khms_states, transition->to);
  to_state = k == kh_end(khms_states) ? NULL : kh_val(khms_states, k);
  transition->depth = to_state ? to_state->max_depth : 0;
  transition->depth_heuristic = log((double)transition->depth + 2.0) + 1.0;
}

static void aco_evaporate(void) {
  u32 i;

  aco_ensure_state_pheromones();

  for (i = 0; i < aco_state_pheromone_count; i++) {
    aco_state_pheromones[i] *= (1.0 - ACO_EVAPORATION_RATE);
    if (aco_state_pheromones[i] < ACO_MIN_PHEROMONE) aco_state_pheromones[i] = ACO_MIN_PHEROMONE;
  }

  for (i = 0; i < aco_transition_count; i++) {
    aco_transitions[i].pheromone *= (1.0 - ACO_EVAPORATION_RATE);
    if (aco_transitions[i].pheromone < ACO_MIN_PHEROMONE) aco_transitions[i].pheromone = ACO_MIN_PHEROMONE;
  }
}

static void aco_deposit_state(u32 state_id, double reward) {
  u32 idx;

  if (reward <= 0.0) return;

  aco_ensure_state_pheromones();
  idx = get_state_index(state_id);
  if (idx >= state_ids_count) return;

  aco_state_pheromones[idx] += reward;
  if (aco_state_pheromones[idx] > ACO_MAX_PHEROMONE) aco_state_pheromones[idx] = ACO_MAX_PHEROMONE;
}

static void aco_reward_transition(aco_transition_t *transition, double reward, u8 new_code_cov, u8 new_state_cov, u8 unique_crash) {
  if (!transition || reward <= 0.0) return;

  transition->successes++;
  if (new_code_cov) transition->code_discoveries++;
  if (new_state_cov) transition->state_discoveries++;
  if (unique_crash) transition->unique_crashes++;

  transition->pheromone += reward;
  if (transition->pheromone > ACO_MAX_PHEROMONE) transition->pheromone = ACO_MAX_PHEROMONE;
}

static void aco_penalize_transition(aco_transition_t *transition, double penalty_factor) {
  if (!transition) return;

  transition->failures++;
  transition->pheromone *= penalty_factor;
  if (transition->pheromone < ACO_MIN_PHEROMONE) transition->pheromone = ACO_MIN_PHEROMONE;
}

static double aco_edge_weight(aco_transition_t *transition) {
  double reliability;
  double novelty;
  ...
  reliability = ((double)transition->successes + 1.0) / ((double)transition->failures + 1.0);
  novelty = 1.0 / sqrt((double)transition->traversals + 1.0);

  return pow(transition->pheromone, ACO_ALPHA) * pow(transition->depth_heuristic * reliability * novelty, ACO_BETA);
}

static u32 aco_choose_edge(u32 from_state, u8 use_from_filter) {
  double total = 0.0, pick, acc = 0.0;
  double *weights;
  u32 i, eligible = 0, chosen = aco_transition_count;

  if (aco_transition_count == 0) return aco_transition_count;
  ...
  for (i = 0; i < aco_transition_count; i++) {
    ...
    weights[i] = aco_edge_weight(&aco_transitions[i]);
    total += weights[i];
    eligible++;
  }

  if (!eligible || total <= 0.0) {
    ck_free(weights);
    return aco_transition_count;
  }

  pick = ((double)UR(1000000) / 1000000.0) * total;
  for (i = 0; i < aco_transition_count; i++) {
    acc += weights[i];
  ...
  if (chosen == aco_transition_count) {
    for (i = aco_transition_count; i > 0; i--) {
      if (weights[i - 1] > 0.0) {
        chosen = i - 1;
        break;
      }
...
static u32 aco_pick_random_edge(u32 from_state, u8 use_from_filter) {
...
static u32 aco_select_next_state(void) {
  ...
  if (aco_transition_count == 0) return aco_fallback_state();

  epsilon = ACO_EPSILON_MIN + (ACO_EPSILON_START - ACO_EPSILON_MIN) / (1 + (u32)(queue_cycle / 2));
  if (aco_stagnation > ACO_STAGNATION_LIMIT) epsilon = ACO_EPSILON_START;

  explore = UR(100) < epsilon;
  edge_idx = explore ? aco_pick_random_edge(aco_current_state_id, 1) : aco_choose_edge(aco_current_state_id, 1);

  if (edge_idx == aco_transition_count) {
    edge_idx = explore ? aco_pick_random_edge(0, 0) : aco_choose_edge(0, 0);
  }

  if (edge_idx == aco_transition_count) return aco_fallback_state();

  aco_last_from_state_id = aco_transitions[edge_idx].from;
  aco_last_to_state_id = aco_transitions[edge_idx].to;
...
static void aco_register_observed_transitions(void) {
  u32 i;
...
static void aco_update_from_feedback(u8 fault, u8 new_queue_entry) {
  double reward = 0.0;
  double penalty_factor;
  u8 new_code_cov;
  u8 valid_path;
  u32 i;
  aco_transition_t *selected_transition = NULL;

  if (!state_aware_mode) return;

  new_code_cov = new_queue_entry && aco_last_code_cov_new;

  valid_path = new_code_cov || aco_last_state_new || aco_last_unique_crash;

  if (new_code_cov) reward += ACO_CODE_REWARD;
  if (aco_last_state_new) reward += ACO_STATE_REWARD;
  if (aco_last_unique_crash) reward += ACO_UNIQUE_CRASH_REWARD;

  aco_evaporate();
  aco_register_observed_transitions();

  if (aco_last_from_state_id || aco_last_to_state_id) {
    selected_transition = aco_get_transition(aco_last_from_state_id, aco_last_to_state_id);
    aco_refresh_transition_depth(selected_transition);
  }
  ...
  if (valid_path) {
    ...
    if (selected_transition) aco_reward_transition(selected_transition, reward * 0.5, new_code_cov, aco_last_state_new, aco_last_unique_crash);
  } else {
    penalty_factor = fault == FAULT_TMOUT ? ACO_TIMEOUT_PATH_PENALTY : ACO_INVALID_PATH_PENALTY;
    aco_stagnation++;
  ...
  aco_updates++;
  aco_log_update(aco_last_from_state_id, aco_last_to_state_id, reward, new_code_cov, aco_last_state_new, aco_last_unique_crash, valid_path, selected_transition);
}
...
void destroy_ipsm()
{
  ...
  ck_free(state_ids);
  ck_free(aco_state_pheromones);
  ck_free(aco_transitions);
  if (aco_log_file) fclose(aco_log_file);
}
...
void update_state_aware_variables(struct queue_entry *q, u8 dry_run)
{
  ...
  for(i = 0; i < state_count; i++) {
    ...
    k = kh_get(hms, khms_states, state_id);
    if (k != kh_end(khms_states)) {
      state_info_t *cur_state = kh_val(khms_states, k);
      if (cur_state->max_depth < i + 1) cur_state->max_depth = i + 1;
    }
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  u8 new_queue_entry = save_if_interesting(argv, out_buf, len, fault);
  queued_discovered += new_queue_entry;
  aco_update_from_feedback(fault, new_queue_entry);
